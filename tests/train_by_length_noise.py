...
def refine_visual_model(dataset: HTRDataset,
    ...
    # -----------------------------------------------------------------
    # Inject label noise by replacing transcriptions with random ones.
    # -----------------------------------------------------------------
    false_cnt = min(N_FALSE, len(subset_idx))
    if false_cnt > 0:
        false_idx = random.sample(subset_idx, false_cnt)
        print(f"[Noise] Injecting {false_cnt} false labels out of {len(subset_idx)} selected.")
        for i in false_idx:
            original_transcription = transcrs[i]
            while True:
                j = random.randrange(len(transcrs))
                if i != j and transcrs[j] != original_transcription:
                    break
            
            dataset.transcriptions[i] = transcrs[j]
            img_path, _ = dataset.data[i]
            dataset.data[i] = (img_path, transcrs[j])
            print(f"[Noise] Replaced GT '{original_transcription}' with '{transcrs[j]}' at idx {i}")
