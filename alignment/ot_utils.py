"""Optimal Transport utilities used during alignment."""
...
def calculate_ot_projections(
    ...
    *,
    ...
    if unbalanced:
        T = ot.unbalanced.sinkhorn_unbalanced(pa, pb, M, reg, reg_m, **sinkhorn_kwargs)
    else:
...
class OTAligner:
    ...
    def __init__(
        ...
        *,
        ...
        self.projectors = projectors if isinstance(projectors, Sequence) else [projectors]
        ...
        assert agree_threshold <= len(self.projectors), "agree_threshold bigger than ensemble size"
    ...
    def _get_projector_outputs(self):
        ...
        assert len(self.dataset) == feats_all.size(0) == aligned_all.size(0), "Feature count mismatch"
        ...
        for proj in self.projectors:
            ...
            assert proj_feats.shape[1] == self.word_embs.shape[1], "Embedding dim mismatch"
    ...
    def _log_results(
        ...
        if self.k > 0 and chosen_list:
            correct_new = sum(
                self.dataset.external_words[self.dataset.aligned[i].item()] == self.dataset.transcriptions[i].strip()
                for i in chosen_list
            )
            print(
                f"[Align] newly pseudo-labelled {len(chosen_list)} items; [Align] round accuracy {correct_new}/{len(chosen_list)} ({correct_new/len(chosen_list):.1%})"
            )
        ...
        if aligned_idx.numel():
            ...
            print(
                f"[Align] cumulative accuracy: {correct_tot}/{aligned_idx.numel()} ({correct_tot/aligned_idx.numel():.1%}) – {aligned_idx.numel()} of {len(self.dataset)} samples aligned"
            )

    def align(self) -> Tuple[torch.Tensor, torch.Tensor, torch.Tensor]:
        out = self._get_projector_outputs()
        chosen = self._select_candidates(out["counts"], out["dist_matrix"], out["plan"], out["aligned_all"])
