...
The script trains the projector for 2000 epochs and halves the learning rate every 500 epochs.
...
* **load_vocab()** – load character-to-index and index-to-character dictionaries from `htr_base/saved_models/c2i.pkl` and `i2c.pkl`. Use this helper everywhere to obtain `c2i` and `i2c`.
* **create_vocab()** – generate the default vocabulary pickles used by `load_vocab()`.
